...
base_model_dir = '/home/michele.maggini/XAI_HIPP/models/FT'

# Define base output directory for all results
base_output_dir = '/home/michele.maggini/XAI_HIPP/IG/results_FT'
...
PERCENTILE_THRESHOLDS = [75, 85, 90, 95]  # Analyze top 25%, 15%, 10%, and 5% most important tokens
...
model_names = [os.path.join(base_model_dir, d) for d in os.listdir(base_model_dir) if os.path.isdir(os.path.join(base_model_dir, d))]

# Process each model only if model_names and dataset_chunks are not empty
if model_names and dataset_chunks:
    for model_path in model_names:
        model_name = os.path.basename(model_path)  # Use directory name as model identifier
        ...
        try:
            ...
            for chunk_idx, df_chunk in enumerate(dataset_chunks):
                ...
                for batch_start in range(0, len(df_chunk), BATCH_SIZE):
                    ...
                    for idx, row in tqdm(
                        ...
                        try:
                            # Check if tokenized text exceeds model limit
                            tokens = tokenizer.tokenize(text)
                            if len(tokens) > 512 - 2:  # Account for [CLS] and [SEP]
                                # Skip this sample if it's still too long even after truncation
                                continue
                            ...
                            if attributions is None or not all(
                                isinstance(attr_tuple, tuple) and len(attr_tuple) == 2
                                for attr_tuple in attributions
                            ):
                                continue
            ...
            if results:
                ...
                output_csv = os.path.join(
                    base_output_dir, f"span_attribution_analysis_{model_name.replace('/', '_')}.csv"
                )
                results_df.to_csv(output_csv, index=False)
                print(f"  Results saved to '{output_csv}'")

                # Generate and save diagnostic information using percentiles
                high_attr_samples = get_high_attribution_samples_by_percentile(results_df, percentile=90)
                high_attr_csv = os.path.join(
                    diagnostic_dir, f"high_attribution_samples_{model_name.replace('/', '_')}.csv"
                )
                high_attr_samples.to_csv(high_attr_csv, index=False)
                print(f"  High attribution samples (top 10%) saved to '{high_attr_csv}'")
...
if all_models_results:
        ...
        for model_name in model_names_processed:
            ...
            for trait in all_traits:
                ...
                if len(trait_data) > 0:
                    ...
                    threshold = get_percentile_threshold(trait_data["attribution_ratio"], percentile)
            ...
            if non_trait_ratios:
                threshold_non_trait = get_percentile_threshold(non_trait_ratios, percentile)
                high_non_trait = len([r for r in non_trait_ratios if r >= threshold_non_trait])
                total_non_trait = len(non_trait_ratios)
                proportion_non_trait = high_non_trait / total_non_trait if total_non_trait > 0 else 0
    ...
    for percentile in PERCENTILE_THRESHOLDS:
        ...
        summary_csv_path = os.path.join(
            percentile_dir, 
            f"percentile_{percentile}_summary.csv"
        )
